    ...
    public void setPostInfo(Writeinfo writeinfo){
        ...
        for (int i = 0; i < contentsList.size(); i++) {
            ...
            if(formats.equals("image")){
                ...
                //MediaSource를 생성할때 DataSourceFactory를 넣어줘야 하는데, DataSource는 URI 리소스로부터 데이터를 읽는데 사용
                DataSource.Factory dataSourceFactory = new DefaultDataSourceFactory(context,
                        Util.getUserAgent(context, getResources().getString(R.string.app_name)));

                //비디오데이터를 Uri로 부터 추출해서 DataSource객체 생성
                MediaSource videoSource = new ProgressiveMediaSource.Factory(dataSourceFactory)
                        .createMediaSource(MediaItem.fromUri(Uri.parse(contents)));
